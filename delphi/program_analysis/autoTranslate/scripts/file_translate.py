...
#        print (prog)    
       ...
       # elif root.attrib["id"] in functionList and state.subroutine["tag"] == "function":
        ...
        fopen = {"tag": "open", "args": []}
        for node in root:
            fopen["args"] += parseTree(node, state)
        return [fopen]

    elif root.tag == "keyword-argument":
        karg = {}
        if root.attrib:
            if root.attrib["argument-name"]:
                for node in root:
                    karg[root.attrib["argument-name"]] = parseTree(node, state) 
            else:
                for node in root:
                    karg['test'] = parseTree(node, state)
                    for element in karg['test']:
                        if element.get('type', None) == 'int':
                            new_name = 'UNIT'
                        elif element.get('type', None) == 'char':
                            new_name = 'FILE'
                    karg[new_name] = karg.pop('test')
            return [karg]
        else:
            return []
   
    elif root.tag == "close":
        cls = {"tag": "close", "fhandle": []}
        for node in root:
           cls["fhandle"] += parseTree(node, state) 
        return [cls]
        ...
        for node in root:
#            print (node)
            if node.tag == "io-controls":
                controls = parseTree(node, state)
                fread["fhandle"] = [controls[0]]
            elif node.tag == "inputs":
                fread["args"] = parseTree(node, state)
        return [fread]
