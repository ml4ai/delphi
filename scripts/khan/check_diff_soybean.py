...
def PETPT(msalb, srad, tmax, tmin, xhlai):
    ...
    for i in range(len(xhlai)):
        if xhlai[i] < 0.0:
            ...
            albedo[i] = 0.23 - (0.23 - msalb)*np.exp(-0.75*xhlai[i])
...
#def PETASCE(msalb, canht, doy, meevp, tdew, windht,
#        windrun, xlat, xelev, srad, tmax, tmin, xhlai):
    ...
    for i in range(len(xhlai)):
        ...
        #rhmin[i] = max(20.0, min(80.0, ea/emax[i]*100.0))
    ...
    for i in range(len(xhlai)):
        ...
        if meevp =='A':
            ...
            kcmax[i] = max((1.2+(0.04*(wnd-2.0)-0.004*(rhmin[i]-45.0))
    ...
    for i in range(len(xhlai)):
        if kcb[i] < kcbmin:
            ...
            fc[i] = ((kcb[i]-kcbmin)/(kcmax[i]-kcbmin))**(1.0+0.5*canht)
...
#y2 = [PETASCE(msalb, canht, doy, meevp, tdew, windht, windrun, xlat, xelev,
#    *args) for args in product(srad, tmax, tmin, xhlai)]
x = np.arange(len(y1))
