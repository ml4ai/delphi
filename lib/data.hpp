...
double get_data_value(string indicator,
                      string country = "",
                      string state = "",
                      int year = 2012,
                      int month = 1,
                      string unit = "")
{
  ...
  string query = "select Unit,Value from indicator where `Variable` like '" + indicator + "'";
  string check_q;
  if (!country.empty())
  {
  ...
  string final_query = query + " and `Year` is '" + std::to_string(year) + "' and `Month` is '" + std::to_string(month) + "'";
