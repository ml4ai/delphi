...
y2 = [PETASCE(*args, canht, doy, meevp, tdew, windht, windrun, xlat, xelev) for args in product(msalb, srad, tmax, tmin, xhlai)]
x1 = np.arange(len(y1))
#x2 = np.arange(len(y2))

print(len(msalb)*len(srad)*len(tmax)*len(tmin)*len(xhlai))
print(len(y1))
print(len(y2))
print(y2)
