...
class Experiment {
  ...
  static void runProjectionExperiment(Database* sqlite3DB,
    ...
    FormattedProjectionResult causemos_experiment_result =
        G.run_causemos_projection_experiment_from_json_string(request.body());
  ...
  mux.handle("create-model")
      .post([&sqlite3DB](served::response& response,
        ...
        */
        ...
        sqlite3DB->insert_into_delphimodel(json_data["id"],
                                           G.serialize_to_json_string(false));
  ...
  */
  ...
  mux.handle("/models/{modelId}/training-progress")
      .get([&sqlite3DB](served::response& res, const served::request& req) {
        ...
        // output["progressPercentage"].dump(), but the specification also calls
        // for application/json content, so we send a JSON structure with only
        // that field.
        json foo;
  ...
  mux.handle("/models/{modelId}/edit-indicators")
      .post([&sqlite3DB](served::response& res, const served::request& req) {
        string message =
            "The edit-indicators endpoint is not implemented for Delphi, since "
            "Delphi (as it is currently implemented) needs to retrain the "
            "whole model on every indicator change. This might change in the "
            "future, but for now, the way to update an existing model is to "
  ...
  mux.handle("/models/{modelId}/edit-edges")
      .post([&sqlite3DB](served::response& res, const served::request& req) {
        string message =
            "Edit-edges: Currently implemented as a NOP.  It will soon have "
            "the semantics of changing the prior distribution of the rate of "
            "change of the target node with respect to the source node "
