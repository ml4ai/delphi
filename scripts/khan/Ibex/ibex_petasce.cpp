...
void contract_and_draw(Ctc& c, IntervalVector& box, const char* color)
{
  ...
  for( int i=0; i<n; i++ ){
    ...
    if(strncmp(color,"k[r]",4)){
      ...
      cout << "srad (lb):" << result[i][2].lb() << endl;
      cout << "srad (ub):" << result[i][2].ub() << endl;
      cout << "msalb (lb):" << result[i][3].lb() << endl;
      cout << "msalb (ub):" << result[i][3].ub() << endl;
      cout << "xhlai (lb):" << result[i][4].lb() << endl;
      cout << "xhlai (ub):" << result[i][4].ub() << endl;
...
int main() {
  ...
  Function patm(flag, flag*101.3*pow((293.0-0.0065*10.0)/293.0, 5.26));
  ...
  Function ea(flag, flag*0.6108*exp((17.27*20.0)/(20.0+237.3)));
  ...
  Function dr(flag, flag*(1.0+0.033*cos(2.0*pi/365.0)));

  //Function ldelta(doy, 0.409*sin(2.0*pi/365.0*doy-1.39));
  Function ldelta(flag, flag*0.409*sin(2.0*pi/365.0-1.39));
  //Function ws(xlat, doy, acos(-1.0*tan(xlat*pi/180.0)*tan(ldelta(doy))));
  Function ws(flag, flag*acos(-1.0*tan(26.63*pi/180.0)*tan(ldelta(flag))));
  ...
  Function ra1(flag, ws(flag)*sin(26.63*pi/180.0)*sin(ldelta(flag)));
  //Function ra2(xlat, doy, cos(xlat*pi/180.0)*cos(ldelta(doy))*sin(ws(xlat,doy)));
  Function ra2(flag, cos(26.63*pi/180.0)*cos(ldelta(flag))*sin(ws(flag)));
  ...
  Function rso(flag, (0.75 + 0.00002*10.0)*ra(flag));
  ...
  Function windsp(flag, flag*400.0*1000/(24*3600));
  ...
  Function wind2m(flag, windsp(flag)*(4.87/log(67.8*3.0 - 5.42)));
  ...
  Function fc0(tmax, xhlai, flag, pow((kcb(xhlai)-kcbmin)/(kcmax1(tmax, xhlai, flag)-kcbmin),1.0+0.5));
  ...
  Function eo(tmax, tmin, srad, msalb, xhlai, flag, kc(tmax, xhlai, flag)*refet(tmax, tmin, srad, msalb, xhlai, flag));
  ...
  Function eonew(tmax, tmin, srad, msalb, xhlai, flag, ibex::max(0.0001, eo(tmax, tmin, srad, msalb, xhlai, flag)));
  ...
  NumConstraint c1(tmax, tmin, srad, msalb, xhlai, flag, eonew(tmax, tmin, srad, msalb, xhlai, flag) <= 10);
  NumConstraint c2(tmax, tmin, srad, msalb, xhlai, flag, eo(tmax, tmin, srad, msalb, xhlai, flag) >= 5.5);

  NumConstraint c3(tmax, tmin, srad, msalb, xhlai, flag, eonew(tmax, tmin, srad, msalb, xhlai, flag) > 10);
  NumConstraint c4(tmax, tmin, srad, msalb, xhlai, flag, eonew(tmax, tmin, srad, msalb, xhlai, flag) < 5.5);
