...
def test_inference(G):
    ...
    s0[f"∂({conflict_string})/∂t"] = 0.1
    G.sample_from_likelihood(s0)
    latent_state_sequence = G.latent_state_sequences[0]
    observed_state_sequence = G.observed_state_sequences[0]
    G.update_log_prior(A)
    G.update_log_likelihood(latent_state_sequence, observed_state_sequence)
    print(G.log_likelihood)

    # Initialize random walk
    A[f"∂({conflict_string})/∂t"][food_security_string] = 2.0
    assert True
