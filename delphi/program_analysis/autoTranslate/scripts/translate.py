...
class ParseState:
    def __init__(self, subroutine=None):
        ...
        self.args = [arg['name'] for arg in self.subroutine['args']] if 'args' in self.subroutine else []
    ...
    if root.tag == 'subroutine' or root.tag == 'program':
        ...
        for node in root:
            if node.tag == 'header' and 'type' not in node.attrib:
                ...
                curIf['header'] = parseTree(node, state)
                #ifs.append(ifStmt)
            elif node.tag == 'body' and ('type' not in node.attrib or node.attrib['type'] != 'else'):
        ...
        for node in root:
            if node.tag == 'operand':
                ...
                if 'operator' in op:
                    newOp = {'tag': 'op', 'operator': node.attrib['operator'], 'left': [op]}
                    op = newOp 
                else:
        ...
        return [{'tag': 'literal', 'type': root.attrib['type'], 'value': root.attrib['value']}]
...
def printAstTree(astFile, tree, blockVal):
    ...
    for node in tree:
        if parentVal != blockVal:
            astFile.write("\tB" + str(parentVal) + " -> B" + str(blockVal) + "\n")
            parentVal = blockVal
        ...
        for key in node:
            if isinstance(node[key], list) and bool(node[key]):
                astFile.write("\tB" + str(parentVal) + " -> B" + str(blockVal) + ' [label="' + str(key) + '"]\n')
                blockVal = printAstTree(astFile, node[key], blockVal)

    return blockVal
        
def printPython1(pyFile, root, sep, add, printFirst, definedVars, globalVars, indexRef):
    ...
    for node in root:
        ...
        if node['tag'] == 'variable':
            if node['name'] not in definedVars and node['name'] not in globalVars:
            ...
            args = []
            printPython(pyFile, node['args'], ", ", "", False, args, globalVars, False)
            pyFile.write("):")
            printPython(pyFile, node['body'], sep+add, add, True, args, globalVars, True)
        elif node['tag'] == 'call':
            pyFile.write("{0}(".format(node['name']))
            printPython(pyFile, node['args'], ", ", "", False, [], globalVars, False)
            pyFile.write(")")
        elif node['tag'] == 'arg':
            pyFile.write("{0}".format(node['name']))
            definedVars += [node['name']]
        ...
        #    pass #skip
            ...
            pyFile.write("for ")
            printPython(pyFile, node['header'], "", "", True, definedVars, globalVars, True)
            pyFile.write(":")
            printPython(pyFile, node['body'], sep+add, add, True, definedVars, globalVars, True)
        elif node['tag'] == 'index':
            #pyFile.write("{0} in range({1}, {2}+1)".format(node['name'], node['low'], node['high']))
            pyFile.write("{0}[0] in range(".format(node['name']))
            printPython(pyFile, node['low'], "", "", True, definedVars, globalVars, True)
            pyFile.write(", ")
            printPython(pyFile, node['high'], "", "", True, definedVars, globalVars, True)
            pyFile.write("+1)")
        elif node['tag'] == 'if':
            pyFile.write("if ")
            printPython(pyFile, node['header'], "", "", True, definedVars, globalVars, True)
            ...
            if 'else' in node:
                pyFile.write(sep)
                pyFile.write("else:")
                printPython(pyFile, node['else'], sep+add, add, True, definedVars, globalVars, True)
        elif node['tag'] == 'op':
            if indexRef == False:
                pyFile.write('[')
            if 'right' in node:
                pyFile.write('(')
                printPython(pyFile, node['left'], "", "", True, definedVars, globalVars, True)
                ...
                printPython(pyFile, node['right'], "", "", True, definedVars, globalVars, True)
                pyFile.write(')')
            else:
                pyFile.write("{0}".format(node['operator']))
                pyFile.write('(')
            ...
            printPython(pyFile, node['target'], "", "", False, definedVars, globalVars, True)
            pyFile.write(" = ")
            printPython(pyFile, node['value'], "", "", False, definedVars, globalVars, True)
...
def main():
    ...
    parser.add_argument('-g', '--gen', nargs='*', help='Routines for which dependency graphs should be generated')
    parser.add_argument('-f', '--files', nargs='+', required=True, help='A list of AST files in XML format to analyze')
