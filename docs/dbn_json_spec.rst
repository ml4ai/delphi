...
Next, the <function_type> is the string representing which of the four types the function belongs to (the types are described in more detail, below): “assign”, “condition”, “container”, “loop_plate”.  In the case of a loop_plate, we will name the specific loop using the generic name “loop”, and include a number of there is more than one loop.
...
Assign:	
An assignment of the variable UPDATE_EST__YIELD_EST in the context of function UPDATE_EST:
UPDATE_EST__assign___UPDATE_EST__YIELD_EST
Condition:
A condition within the function UPDATE_EST assigning the (inferred) boolean variaaible IF_1:
UPDATE_EST__condition___IF_1
Container:
A container function called CROP_YIELD:
...
NOTE: There is some redundancy in the above examples between the <enclosing_context> of the name of the function and the <enclosing_context> of the name of variable, however I think both are needed.
Top-level DBN-JSON specification:
The top-level structure to the DBN-JSON is the <dbn_json_spec> and is itself a JSON attribute-value list, with the following schema definition:
...
The “dateCreated” attribute is a string representing the date+time that the current DBN-JSON was generated (to represent versioning).

FUTURE: may need to extend “name” value to accommodate multiple source files.
FUTURE: may also be desirable to add attribute to represent the program_analysis code version used to generate (as presumably the program_analysis code could evolve and have different properties) -- although “dateCreated” may be sufficient.
...
FUTURE: May also need to accommodate other structures (How far can this go?  Unions, composite data structures, classes?).
FUTURE: I see augmenting the domain specification to also allow representing whether there are bounds on the values (e.g., positive integers, or real values in (0,10], etc. ).  When we move to doing this, the value of "domain" will itself become a new JSON attrval type.

Python values are not statically typed (although python values *are* strongly typed).  However, that's not to say that there is no type specification in python.  Python 3 now provides nascent support for explicit typing (“hints”):
...
If we also want to capture this in program-analysis-generated code, then there is question of how to communicate this in the Python source representation; possibly through the new typing mentioned above; possibly as docstrings in program-analysis-generated code.
...
All three specs will have a "type" attribute that will unambiguously identify which type of function is being specified.  The four types are:
    "assign" | "condition"     # "condition" is a special case of "assign"
    "container"
    "loop_plate"
...
NOTE: For now (20180709), we will NOT be using the Select function; needs revisiting.  Immediate needs

Condition blocks (if-then-else control flow blocks) are decomposed into 3 types of components:
The condition state (what the conditional test of the condition block evaluates to).  In general, this is a boolean variable, whose state is defined by an assign function (and usually more specifically a “condition” type).  There are two ways these get specified in source code:
By a variable (direct name or reference), where the variable state is already determined elsewhere (by another assign function), e.g.:
	if var: ...
In this case, var must itself be a boolean.
The computation of the condition is asserted directly inline, e.g.:
		if var <= 3: ...
In this case, the condition “var <= 3” is itself made into a condition assign function (specified in a <function_assign_spec> with “type” = “condition”), and the resulting state of the condition is represented by an introduced condition boolean variable, such as IF_1.
But the bottom line is that in either case, a <function_assign_spec> will define the function that determines the condition state variable.
The actual conditions themselves.  In the general case, these could be arbitrary code bodies, which could be specified by container functions (described below).  Here we consider several types situations:
Each condition specifies an assign function to the *same* variable.  This makes the overall condition block as really specifying the conditional function determining a single target variable state.
Each condition specifies an assign function to possibly different variables.  In this case, the overall conditional block is asserting the conditions under which only one or a subset of variables get updated.
A condition has another conditional statement.  TODO.  [NOTE: This seems to really just be a generalization of a binary condition to a multivariate switch.]
A condition has a loop_plate statement.  TODO.  [NOTE: Possibly we need loop_plates to have a way to not only specify/reference the initial states of variables, but also final states of variables.]
Finally, there is the actual “wiring” that determines which condition is selected based on the state of the condition state.  This is generally implicit in the syntax of the condition expression, but we need to capture it explicitly to map to DBN variable wiring.

<function_select_spec>[attrval] ::=
	"name" : <function_name>
"type" : "select"
"condition_variable" : <variable_reference> | <variable_name>
"conditions" : list of <condition>

<condition>[attrval] ::=
	"condition_value" : <string>
	"condition" : list of <function_name>

The “condition_value” is possible value of condition_variable and determines which <condition> is selected when condition_variable is in that state.  For now, condition_variable is assumed to be boolean, so condition_values are “true” or “false”.

TODO: how will condition “outcome(s)” be wired to context outside the condition block (e.g., if part of a function return value)?
...
Else
    Return <cond2>
(maybe like would be the way Jon currently handles condition combined with handling a single return value)
...
There will be a container function for each source code function.  For this reason, we need an “input” variable list (of 0 or more variables) as well as an “output” variable.  In Python, a function only returns a value if there is an explicit return expression.  Otherwise it returns None.  How does this work in Fortran?
...
The “input” list of <variable_name>s should list all variables that are set in the scope outside of the loop_plate.  
...
TODO: I think Fortran is restricted to integer values for iteration variables, which would include iteration over indexes into arrays.  Is that right?
