...
void contract_and_draw(Ctc& c, IntervalVector& box, const char* color) {
	...
	for (int i=0; i<n; i++) {
      cout << result[i][0].lb() << result[i][1].lb() << result[i][0].ub() << result[i][1].ub() << endl;
...
int main() {
	...
	NumConstraint c1(x,y,f(x,y)<=25);
	NumConstraint c2(x,y,f(x,y)>=16);
	NumConstraint c3(x,y,f(x,y)>25);
	NumConstraint c4(x,y,f(x,y)<16);
