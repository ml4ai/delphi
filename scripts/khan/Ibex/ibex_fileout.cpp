...
void contract_and_draw(Ctc& c, IntervalVector& box, const char* color)
{
  ...
  for( int i=0; i<n; i++ ){
      ...
      if(strncmp(color,"k[r]",4)){
        ...
        myFile << "tmax (lb):" << result[i][0].lb() << endl;
        //fout << "tmax (ub):" << result[i][0].ub() << endl;
        //fout << "tmin (lb):"  << result[i][1].lb() << endl;
        //fout << "tmin (ub):"  << result[i][1].ub() << endl;
...
int main() {
  ...
  Function eo0(tmax, tmin, msalb, xhlai, srad, eeq(tmax, tmin, msalb, xhlai, srad)*1.1);

  Function eo1(tmax, tmin, msalb, xhlai, srad, chi(tmax - 35, eo0(tmax, tmin, msalb, xhlai, srad), eeq(tmax, tmin, msalb, xhlai, srad)*((tmax-35.0)*0.05 + 1.1))); // tmax > 35
  Function eo2(tmax, tmin, msalb, xhlai, srad, chi(5 - tmax, eo0(tmax, tmin, msalb, xhlai, srad) ,eeq(tmax, tmin, msalb, xhlai, srad)*0.01*exp(0.18*(tmax+20.0)))); // tmax < 5


  //  Function condition(tmax, ibex::max(5-tmax, tmax -35));
  Function eonew(tmax, tmin, msalb, xhlai, srad, chi(tmax - 35, chi(5 - tmax, eo0(tmax, tmin, msalb, xhlai, srad) ,eeq(tmax, tmin, msalb, xhlai, srad)*0.01*exp(0.18*(tmax+20.0))) , eeq(tmax, tmin, msalb, xhlai, srad)*((tmax-35.0)*0.05 + 1.1))); // tmax > 35 && tmin < 5

  Function eo(tmax, tmin, msalb, xhlai, srad, ibex::max(0.0001,eonew(tmax, tmin, msalb, xhlai, srad)));
  ...
  NumConstraint c1(tmax, tmin, msalb, xhlai, srad, eo(tmax, tmin, msalb, xhlai, srad) <= 10.0);
  NumConstraint c2(tmax, tmin, msalb, xhlai, srad, eo(tmax, tmin, msalb, xhlai, srad) >= 5.5);

  NumConstraint c3(tmax, tmin, msalb, xhlai, srad, eo(tmax, tmin, msalb, xhlai, srad) > 10.0);
  NumConstraint c4(tmax, tmin, msalb, xhlai, srad, eo(tmax, tmin, msalb, xhlai, srad) < 5.5);
