...
********************************************************************************
      ...
      subroutine sir_gillespie(beta, gamma, S, I, R, total)
        double precision S, I, R, beta, gamma
        double precision rateInfect, rateRecover, total
        rateInfect = beta * S * I / (S + I + R)
        rateRecover = gamma * I
        total = rateInfect + rateRecover

        if (0.5 < (rateInfect/total)) then
            ! Delta for infection
            S = S - 1
            I = I + 1
        else
...
C           NOTE: these need to not change during the call to sir_gillespie
            n_S_temp = n_S
            n_I_temp = n_I
            n_R_temp = n_R

            call sir_gillespie(beta, gamma, n_S, n_I, n_R, totalRates)
            ...
            do while (sample_idx < Tmax .and. t > samples(sample_idx))
               ...
               call update_mean_var(MeanS, VarS, sample, n_S_temp, runs)
               call update_mean_var(MeanI, VarI, sample, n_I_temp, runs)
               call update_mean_var(MeanR, VarR, sample, n_R_temp, runs)
