...
if __name__ == "__main__":
    ...
    # modules where each module is written out into a separate python file.
    with open(args.out[0], "r") as f:
        python_files = f.read()

    # Python filenames are space separated. Append each one to a list.
    python_file_list = python_files.rstrip().split(" ")

    asts = get_asts_from_files(python_file_list, args.printAst)

    for index, inAst in enumerate(asts):
        # Read the mode_gen file containing all the identifier mappings
        mode_mapperDict = get_index(python_file_list[index][:-3] + ".xml")

        lambdaFile = python_file_list[index][:-3] + "_" + args.lambdaFile[0]
        pgmFile = python_file_list[index][:-3] + "_" + args.PGMFile[0]
        pgm_dict = create_pgm_dict(
            lambdaFile, [inAst], python_file_list[index], mode_mapperDict
        )
