...
like in the autoTranslate bash script, it creates Python object
...
def generate_rectified_xml(
    ...
    rectified_xml, module_files_to_process = rectify.buildNewASTfromXMLString(
        ofp_xml, original_fortran_file, module_log_file_path
    )
...
def generate_outputdict(
    ...
    if not tester_call:
        print(
            "+Generating pickle file: Func: <xml_to_py>, Script: "
            "<translate.py>"
        )
        try:
...
def parse_args():
    ...
    parser.add_argument(
        "-m", "--moduleLog", nargs="*", help="Module log file name."
    )
...
def fortran_to_grfn(
    ...
    mode_mapper_dict = generator.analyze(
        mode_mapper_tree, module_log_file_path
    )
