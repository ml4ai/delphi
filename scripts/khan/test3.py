...
y1 = [vfunc1(msalb, *args) for args in product(srad, tmax, tmin, xhlai)]
...
y2 = [vfunc2(msalb, canht, doy, meevp, tdew, windht, windrun, xlat, xelev, *args) for args in product(srad, tmax, tmin, xhlai)]
...
colors = np.random.rand(len(y2[0]))
#colors = cm.rainbow(np.linspace(0, 1, len(y2[0])))
print('colors :', colors)
...
#    val = [item[i] for item in y2]
    #print(val)
#    plt.scatter(x2+1, val, label = 'PETASCE', c = colors[i])
#    plt.xlabel('Parameter set')
#    plt.ylabel(r'EO ($\times 10^4$)')
#    plt.legend()
   #plt.show()
