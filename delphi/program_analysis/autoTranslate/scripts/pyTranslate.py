...
class PrintState:
    ...
    def copy(self, sep=None, add=None, printFirst=None, definedVars=None, globalVars=None, indexRef=None, varTypes = None):
        return PrintState(
...
def printSubroutine(pyFile, node, printState):
    ...
    args = []
    printAst(pyFile, node['args'], printState.copy(sep=", ", add="", printFirst=False, definedVars=args, indexRef=False))
    pyFile.write("):")
    printAst(pyFile, node['body'], printState.copy(sep=printState.sep+printState.add, printFirst=True, definedVars=args, indexRef=True))
...
def printCall(pyFile, node, printState):
    ...
    printAst(pyFile, node['args'], printState.copy(sep=", ", add="", printFirst=False, definedVars=[], indexRef=False))
    pyFile.write(")")
...
def printVariable(pyFile, node, printState):
    if node['name'] not in printState.definedVars and node['name'] not in printState.globalVars:
        printState.definedVars += [node['name']]
        if(node['type'] == 'INTEGER'):
            initVal = 0
            varType = 'int'
            ...
            print('unrecognized type {0}'.format(node['type']))
            sys.exit(1)
        pyFile.write("{0}:typing.List[{1}] = [{2}]".format(node['name'], varType, initVal))
    else:
        printState.printFirst = False

def printDo(pyFile, node, printState):
    pyFile.write("for ")
    printAst(pyFile, node['header'], printState.copy(sep="", add="", printFirst=True, indexRef=True))
    pyFile.write(":")
    printAst(pyFile, node['body'], printState.copy(sep=printState.sep+printState.add, printFirst=True, indexRef=True))
...
def printIndex(pyFile, node, printState):
    ...
    printAst(pyFile, node['low'], printState.copy(sep="", add="", printFirst=True, indexRef=True))
    pyFile.write(", ")
    printAst(pyFile, node['high'], printState.copy(sep="", add="", printFirst=True, indexRef=True))
    pyFile.write("+1)")
...
def printIf(pyFile, node, printState):
    ...
    if 'else' in node:
        ...
        pyFile.write("else:")
        printAst(pyFile, node['else'], printState.copy(sep=printState.sep+printState.add, printFirst=True, indexRef=True))
...
def printOp(pyFile, node, printState):
    ...
    if 'right' in node:
        pyFile.write('(')
        printAst(pyFile, node['left'], printState.copy(sep="", add="", printFirst=True, indexRef=True))
        ...
        printAst(pyFile, node['right'], printState.copy(sep="", add="", printFirst=True, indexRef=True))
        pyFile.write(')')
    else:
        pyFile.write("{0}".format(node['operator']))
        pyFile.write('(')
...
def printAssignment(pyFile, node, printState):
    printAst(pyFile, node['target'], printState.copy(sep="", add="", printFirst=False, indexRef=True))
    pyFile.write(" = ")
    printAst(pyFile, node['value'], printState.copy(sep="", add="", printFirst=False, indexRef=True))
